...
from config.settings import AreaConfig, USE_POSTGIS
...
class OSMPreprocessor:
    ...
    def extract_edges(self):
        ...
        if USE_POSTGIS:
            table_name = f"edges_{self.area}"
            db = DatabaseManager(table_name=table_name)
            db.save(graph)
            print(f"Edge data saved to PostGIS table '{table_name}' with {len(graph)} rows")
        else:
        ...
        gpkg_path = self.output_path.replace(".parquet", "_vis.gpkg")
